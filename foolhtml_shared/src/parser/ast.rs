...
fn add_cont_block_line<'a>(elem: &mut Elem, val: &'a str) {
    match elem.cont {
        Some(ref mut cont_enum) => match cont_enum {
...
mod tests {
    ...
    fn parses_two_children() {
        test_str_elems!(
            ...
            [Elem::from_ta_ch(
                "hello",
                node_el_vec![Elem::from_ta("world"), Elem::from_ta("today")]
            )]
        );
    }
    ...
    fn parses_two_level_children() {
        test_str_elems!(
            ...
            [Elem::from_ta_ch(
                ...
                node_el_vec![
                    Elem::from_ta("world"),
                    Elem::from_ta_ch("today", node_el_vec![Elem::from_ta("tomorrow")]),
                    Elem::from_ta("always")
                ]
            )]
        );
    ...
    fn parses_single_class() {
        test_str_elems!(
            "hello.world-fam",
            [Elem::from_ta_cl("hello", string_vec!["world-fam"])]
        );
    }
    ...
    fn parses_single_id() {
        test_str_elems!(
            "hello#world-class-nr1",
            [Elem::from_ta_id("hello", "world-class-nr1")]
        );
    }
    ...
    fn parses_multi_line_content() {
        test_str_elems!(
            "hello:\n  bon\n  jour",
            [Elem::from_ta_cob("hello", string_vec!["bon", "jour"])]
        );
    }
    ...
    fn parses_include() {
        ...
        assert_eq!(
            output,
            vec![Node::INCLUDE(Include {
                path: "my/include".into()
            })]
        );
    }
}
